...
import com.ipassistat.ipa.comm.interfaces.PageIndicator;

/***
 * 圆形的可以绑定在ViewPager上的指示器
 * com.ipassistat.ipa.view.banner.CirclePageIndicator
 * @author 时培飞 
 * Create at 2015-4-21 上午9:41:19
 */
public class CirclePageIndicator extends View implements PageIndicator {
	...
	private float gap = 4; // 设置两个 原点之间 的距离 = mRadius * gap;

	/**
	 * @return 两点之间的距离 = mRadius * gap ；
	 */
	public float getGap() {
		return gap;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		final float threeRadius = mRadius * gap; // 控制 圆圈间的距离
		...
		float pageFillRadius = mRadius; // 半径
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			...
			if (pageFillRadius != mRadius) {
				canvas.drawCircle(dX, dY,
						mRadius - mPaintStroke.getStrokeWidth() / 2,
						mPaintStroke);
			}
		}
