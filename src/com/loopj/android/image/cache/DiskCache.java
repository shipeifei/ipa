...
public class DiskCache {
	...
	public static final String DISK_CACHE_DIR = ConfigInfo.DEFAULT_DISK_DIR_CACHE;
	...
	public static DiskCache openCache() {
		...
		if (!cacheDir.exists()) {
			boolean sueccess = cacheDir.mkdirs();
			File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + DISK_CACHE_DIR, ".nomedia");
			LogUtil.outLogDetail("path:" + file);
			try {
				file.createNewFile();
			} catch (IOException e) {
				e.printStackTrace();
			}
			// LogUtil.outLogDetail("create cache dir：" + sueccess);
		}
		if (cacheDir.isDirectory() && cacheDir.canWrite() && Utils.getUsableSpace(cacheDir) > DISK_CACHE_SIZE) {
			return new DiskCache(cacheDir);
		} else {
			// LogUtil.outLogDetail("create cache dir：clearCache");
			// clearCache(cacheDir);
			return new DiskCache(cacheDir);
		}
	...
	public String get(String key) {
		...
		if (new File(existingFile).exists()) { return existingFile; }
	...
	public boolean containsKey(String key) {
		final String existingFile = createFilePath(mCacheDir, key);
		if (new File(existingFile).exists()) { return true; }
		return false;
	}
